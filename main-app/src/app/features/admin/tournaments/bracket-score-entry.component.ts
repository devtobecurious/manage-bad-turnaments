...
import { SetScore, validateSet, validateMatch } from '../../../core/models/match.model';
...
@Component({
  selector: 'app-bracket-score-entry',
  standalone: true,
  imports: [FormsModule],
  template: `
    <!-- Modal backdrop -->
    ...
    >
      ...
      >
        ...
        <div class="px-6 pt-6 pb-4 border-b border-gray-100">
          <h2 class="text-lg font-semibold text-gray-900">Saisie du score — Tableau final</h2>
        ...
        <div class="px-6 py-4 space-y-5">
          ...
          <div class="flex items-center gap-3">
            <input
              id="forfeit-toggle-bracket"
              type="checkbox"
              [checked]="forfeitMode()"
              (change)="toggleForfeit()"
              class="w-4 h-4 text-indigo-600 rounded border-gray-300"
            />
            <label for="forfeit-toggle-bracket" class="text-sm font-medium text-gray-700 cursor-pointer">
              Forfait
            </label>
          </div>
            ...
            <div class="space-y-2">
              ...
              <div class="flex flex-col gap-2">
                <label class="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    name="forfeit-participant-bracket"
            ...
            <div class="space-y-3">
              ...
              @for (set of sets(); track $index; let i = $index) {
                <div class="grid grid-cols-[1fr_2rem_1fr_2.5rem] gap-2 items-center">
                  ...
                  <!-- Remove set button (only for non-first sets, last set) -->
...
export class BracketScoreEntryComponent {
  ...
  private readonly parsedSets = computed((): SetScore[] => {
    return this.sets()
  ...
  readonly currentWinnerName = computed((): string | null => {
    if (this.forfeitMode()) {
      ...
      return forfeitId === m.participantA?.id
        ? m.participantB?.name ?? null
        : m.participantA?.name ?? null;
    }
    const parsed = this.parsedSets();
    if (parsed.length !== this.sets().length) return null;
    const result = validateMatch(parsed);
    ...
    const m = this.match();
    if (winsA >= 2) return m.participantA?.name ?? null;
    if (winsB >= 2) return m.participantB?.name ?? null;
    return null;
  });
  ...
  async save(): Promise<void> {
    ...
    try {
      ...
      await this.bracketService.updateBracketMatchScore(
        this.tournamentId(),
        m.id,
        sets,
        forfeitId
